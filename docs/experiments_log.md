...
**Tested:** GPT-4o-mini, GPT-4.1-nano, Claude Haiku, Qwen2.5-14B, Qwen3-8B, Qwen3-14B.
...
**Configs tested:**
- `homo_gpt`, `homo_haiku`, `homo_qwen3_14b`, `homo_qwen3_8b`: does decomposition help this model?
...
**Key findings:**
- **Decomposition helps weak models, hurts the strongest open-source one (vertical):**  
  LLaMA-3.1-8B +37 Combined (10→47), GPT-4o-mini +24 (54→79), Qwen3-4B +19 (48→67), Qwen3-8B +10 (42→52), Haiku slightly better but almost flat (~91, already near ceiling), Qwen3-14B **−5** (81→76).
  The weaker the base model, the more decomposition helps. LLaMA-3.1-8B's gain is the largest
...
- **Decomposition helps weak models, hurts the strongest open-source ones (vertical):**  
  ...
  Qwen3-14B **−5** (81→76), Phi-4 14B **−12** (67→55). The weaker the base model, the more decomposition helps. LLaMA-3.1-8B's gain is the largest. Phi-4 14B regressed even more than 
  Qwen3-14B, suggesting the "decomposition hurts strong models" rule generalizes across families at the 14B scale
...
- **Hetero "specialize by strength" did NOT pay off (horizontal):**  
  ...
  - `hetero_haiku_gpt` (87.7) < `homo_haiku` (91.4). Downgrading ResponseGen alone cost 3.7 pts
...
- **DST is the bottleneck:**  
  ...
  The Qwen pair confirms it: `hetero_qwen3_14b_qwen3_8b` (54.2) ≈ `homo_qwen3_8b` (52.2). A strong DST barely rescues a weak ResponseGen, but a weak DST drags everything down

- **Hallucination diverges by model class:**  
  - API models drop sharply (GPT 5.9→3.1, Haiku 4.4→0.8), so placeholder discipline (delex) works
  - Qwen3-14B stays flat (2.7→3.0), little room to gain. Qwen3-8B rises (9.7→11.2), too small to follow the placeholder rule
...
**Mechanism behind the split:** Splitting gives up context-sharing in exchange for focus per call. Most models gain because each sub-task fits their capacity better, 
  especially weak ones (GPT, Qwen3-8B). Qwen3-14B is the only loser: it was already grounded and used the full context productively in one pass.
...
**Configs at a glance:**
- `ft_homo_llama32_3b`: smallest viable base, can a 3B model after FT close the gap with larger zero-shot or decomposed baselines?
- `ft_homo_qwen3_8b`: mid-size, tests if FT lets a small base catch larger zero-shot ones
- `ft_homo_qwen3_14b`: strongest open-source base, tests if FT still helps an already-strong model

**Key findings:**

- **Fine-tuning helps weak models, hurts the strong one (vertical Exp2 → Exp3):**  
  LLaMA-3.2-3B 23 → 58 (**+35**), Qwen3-8B 52 → 62 (**+10**), LLaMA-3.1-8B 47 → 55 (**+8**), Qwen3-14B 76 → 71 (**−5**). Same pattern as decomposition in Exp2. 
  The gain comes from filling capacity gaps, not from teaching strong models new tricks. The smaller and weaker the base model, the more fine-tuning helps

- **Fine-tuning helps "undisciplined" weak models, hurts already-disciplined ones (vertical Exp2 → Exp3):**  
  LLaMA-3.2-3B 23 → 58 (**+35**), Phi-4 14B 55 → 71 (**+16**), Qwen3-8B 52 → 62 (**+10**), LLaMA-3.1-8B 47 → 55 (**+8**), Qwen3-14B 76 → 71 (**−5**), Qwen3-4B 67 → 54 (**−13**).
  The rule is NOT just "small models gain more": it depends on **Exp2 hallucination level**. Models with high Hall% in Exp2 (LLaMA-3.2-3B 14.2%, Phi-4 14B 9.2%, Qwen3-8B 11.2%) gain from FT teaching them structure. 
  Models already disciplined in Exp2 (Qwen3-14B 3.0%, Qwen3-4B 3.2%) have no room to gain on placeholder discipline and only suffer from FT noise. 
  Phi-4 14B is a strong confirmation: at 14B scale, FT can still help if the base wasn't already disciplined. Qwen3-4B is the clearest counter-example to a naive "smaller = bigger FT gain" claim
...
- **DST closes the API gap. ResponseGen does not (with one exception):**  
  ...
  Phi-4 14B is the exception: ft_homo_phi4_14b reaches Combined 70.8, the highest open-source score in Exp3 and within ~21 pts of homo_haiku. Inform recovers from 61.3 (Exp2) to 
  66.7 (Exp3), Success from 43.5 to 57.5, BLEU triples (2.83 → 8.74). The combination of weak Exp2 (decomposition hurt it) plus strong base capacity gives FT the most room to recover
...
- **Cross-family pattern confirmed:** Both LLaMA (3B, 8B) and Qwen (4B, 8B, 14B) families show the same trajectory: decomposition gains in Exp2, further DST gains in Exp3 via FT, 
  but rising hallucination eroding end-to-end scores. The "FT helps undisciplined models" (and not "FT helps weak more than strong") rule holds across families, with LLaMA more 
  vulnerable to hallucination explosion than Qwen
...
- **Cross-family pattern confirmed across three families:** LLaMA (3B, 8B), Qwen (4B, 8B, 14B), and Phi-4 (14B) all show the same trajectory shape: DST gains in Exp3 via FT, with rising 
  hallucination eroding end-to-end scores. The "FT helps undisciplined models" rule holds across families.
...
**Mechanism behind the result:** Fine-tuning excels at structured outputs (DST: slots, JSON) where ground truth is clean and the task is narrow. It struggles 
  on open-ended generation (ResponseGen) because the delexicalized training data is noisy as multi-domain turns and imperfect string matching leave real entity names in 
  some training outputs. Small models pick up the noise and break placeholder discipline at inference.
